...
const navLinks = [
  ...
  { href: "/SubmitTestimony", label: "Submit Testimony" },
        ...
        <nav className="hidden md:flex gap-6 text-md  items-center font-medium">
          {navLinks.map(({ href, label }) => (
            <Link
              ...
              className={`${
                pathname === href
                  ? "text-gray-600 border-b-2 decoration-0  border-green-600"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.nav
            ...
            className="md:hidden bg-white min-h-screen dark:bg-gray-800 border-t border-green-600 dark:border-gray-700 px-4 py-4 space-y-4 text-base font-medium text-left"
          >
            {navLinks.map(({ href, label }) => (
              <Link
                key={href}
                ...
                className={`block w-full ${
                  ...
                  label === "Gift"
                    ? "bg-gray-800 text-gray-100 px-4 py-2 rounded-md hover:bg-green-500  border-1 border-white transition"
                    : ""
                }`}
              >
                {label}
