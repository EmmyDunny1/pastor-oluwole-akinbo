"use client";
import "server-only"; // ensure this is a client component
...
/* === edit these book entries to match your data/images === */
const BOOKS: Book[] = [
  {
    id: 1,
...
/* Fallback image helper using next/image but controlled src state */
function FallbackImage({
  ...
  return (
    // Using Next Image but switching src to placeholder on error.
    <Image
      src={imgSrc}
      alt={alt}
      width={width ?? 600}
...
export default function BooksPage(): JSX.Element {
  ...
  useEffect(() => {
    // simple auto-scroll for carousel
  ...
  // split prelaunch from other books and ensure prelaunch appears right after author info
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-[#171717] to-gray-800 text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        ...
        {/* HERO / AUTHOR + SHORT TEXT */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center mb-12">
          {/* Left: Heading + intro (smaller text area per your last instruction) */}
          ...
          {/* Right: large author image (bigger) */}
          <div className="flex justify-end">
            <div className="relative w-full max-w-md lg:max-w-xl transform transition-all duration-500 hover:-translate-y-3">
              ...
              {/* small overlapping info card on top of the image */}
        ...
        {/* Place PRELAUNCH immediately after author info */}
        {prelaunch && (
          <section className="mb-10">
            ...
            >
              ...
              {/* LEFT: text (reduced text area size a bit) */}
              ...
              {/* RIGHT: bigger image area + glowing preorder button below */}
              <div className="w-full lg:w-1/3 z-10 flex flex-col items-center justify-center px-3 py-6">
                ...
                {/* Preorder button (neon / expanding) */}
                <a
                  href={prelaunch.link}
                  className="mt-6 relative inline-flex items-center gap-3 px-8 py-4 rounded-full font-extrabold text-white shadow-xl preorder-neon transition-all duration-300"
                >
        ...
        {/* CAROUSEL: OTHER BOOKS (interactive, auto-scroll) */}
        <section aria-label="Other books carousel" className="mb-12">
          ...
          >
            <div className="flex gap-6 w-max py-4 px-2">
              {/* replicate a cover card for each book (cycle through BOOKS) */}
              {BOOKS.map((b) => (
                <div key={`card-${b.id}`} className="min-w-[260px] flex-shrink-0 bg-[#171717]/60 rounded-2xl p-4 shadow-lg">
                  <div className="rounded-lg overflow-hidden mb-3">
                    <FallbackImage src={b.cover} alt={b.title} width={260} height={360} className="w-full h-40 object-cover" />
                  </div>
                  <div>
            ...
            {/* optional left/right manual controls */}
      ...
      <style jsx>{`
        .neon-halo {
          box-shadow:
            0 0 40px 10px rgba(255, 80, 255, 0.08),
            0 0 80px 20px rgba(120, 60, 255, 0.06),
            0 0 160px 30px rgba(255, 110, 60, 0.04);
        }

        /* Preorder neon button pulse (subtle) */
        .preorder-neon::before {
          content: "";
          position: absolute;
          inset: 0;
          border-radius: 9999px;
          background: linear-gradient(90deg, rgba(255,90,255,0.28), rgba(120,60,255,0.22), rgba(255,110,60,0.22));
          filter: blur(10px);
          z-index: -1;
