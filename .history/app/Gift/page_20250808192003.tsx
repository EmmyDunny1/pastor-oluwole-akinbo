...
export default function Payment() {
  ...
  const handlePayment = (e: React.FormEvent) => {
    e.preventDefault();
    if (window.BudPayPop && formRef.current) {
      ...
      const amountInput = formRef.current.elements.namedItem("amount") as HTMLInputElement | null;
      const key = process.env.NEXT_PUBLIC_BUDPAY_PUBLIC_KEY || "";
      const email = emailInput?.value || "";
      const first_name = nameInput?.value || "";
      const amountValue = amountInput?.value || "";
      if (!key || !email || !first_name || !amountValue) {
        alert("Please fill in all required fields and ensure payment key is set.");
        return;
      }
      window.BudPayPop({
        key,
        email,
        amount: Number(amountValue) * 100, // Budpay expects kobo
        currency: "NGN",
        first_name,
        onClose: () => {},
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-blue-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 py-10 px-4">
      <div className="w-full max-w-md bg-white/90 dark:bg-gray-900/90 rounded-xl shadow-lg p-8">
        ...
        <p className="text-center text-gray-600 dark:text-gray-300 mb-8">Support the celebrant with a monetary gift. Maybe Fill in your details and proceed to payment securely.</p>
